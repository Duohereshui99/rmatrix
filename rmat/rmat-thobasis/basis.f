...
ccccccc
      function D1Laguerre(n,alpha,x) !d/dx L(n,l+0.5,x),alpha=l+0.5
            implicit none
            integer::n
            real*8::alpha
            complex*16::x,D1Laguerre
            D1Laguerre=1d0/x*(n*generalized_laguerre(n,alpha,cmplx(x,kind=8))
     &       -(n+alpha)*generalized_laguerre(n-1,alpha,cmplx(x,kind=8)))
       end function D1Laguerre
ccccccc
      function D2Laguerre(n,alpha,x) !d^2/dx^2 L(n,l+0.5,x),alpha=l+0.5
            implicit none
            integer::n
            real*8::alpha
            complex*16::x,D2Laguerre
            D2Laguerre=1d0/x**2*
     &       ((n**2d0-n)*generalized_laguerre(n,alpha,cmplx(x,kind=8))  
     &   -(2d0*n-2)*(n+alpha)*generalized_laguerre(n-1,alpha,cmplx(x,kind=8))
     &   +(n+alpha)*(n-1+alpha)*generalized_laguerre(n-2,alpha,cmplx(x,kind=8)))
...
ccccccc
      function D2LSTFUN(gamma,m,r)     !s''(r) analytic expression
      implicit none
      real*8::gamma,m
      complex*16::r,D2LSTFUN
      D2LSTFUN=-(1d0/((1d0/r)**m+(1d0/sqrt(r)/gamma)**m))**(2d0+(1d0/m))
     & *((2d0+m)*(1d0/r)**m+(1d0/sqrt(r)/gamma)**m)*(1d0/sqrt(r)/gamma)**m
     & /(4d0*r**2)
      end function
ccccccc
      function D3LSTFUN(gamma,m,r)     !s'''(r) analytic expression
      implicit none
      real*8::gamma,m
      complex*16::r,D3LSTFUN
...
ccccccc
      function D1THOFUNC(n,l,alpha,gamma,m,r)    !d/dr R_{nl}^{THO}(r)
      implicit none
      integer::n,l
      real*8::alpha,gamma,m
      complex*16::r,D1THOFUNC
     ...
     &  +LSTFUN(gamma,m,r)*D2LSTFUN(gamma,m,cmplx(r,kind=8))*
     *  generalized_laguerre(n,l+0.5d0,cmplx(2*alpha*LSTFUN(gamma,m,r)**2,kind=8))                         
     &  ) 
      end function
ccccccc
      function D2THOFUNC(n,l,alpha,gamma,m,r)    !d^2/dr^2 R_{nl}^{THO}(r)
      implicit none
      integer::n,l
      real*8::alpha,gamma,m
      complex*16::D2THOFUNC
      complex*16::r,sr,d1sr,d2sr,d3sr,LL,D1L,D2L
ccccccc
      sr=LSTFUN(gamma,m,cmplx(r,kind=8))
      d1sr=D1LSTFUN(gamma,m,cmplx(r,kind=8))
      d2sr=D2LSTFUN(gamma,m,cmplx(r,kind=8))
      d3sr=D3LSTFUN(gamma,m,cmplx(r,kind=8))
      LL=generalized_laguerre(n,l+0.5d0,cmplx(2d0*alpha*sr**2,kind=8))
      D1L=D1Laguerre(n,l+0.5d0,cmplx(2d0*alpha*sr**2,kind=8))
      D2L=D2Laguerre(n,l+0.5d0,cmplx(2d0*alpha*sr**2,kind=8))
ccccccc
